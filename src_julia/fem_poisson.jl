...
# J.S. Hysing 161008.
    ...
    return u, [ t_grid, t_ptr, t_asm, t_rhs, t_bdr, t_sparse, t_sol ]
...
function rectgrid( n_cx = 10, n_cy = 0, xp = [0 1;0 1] )
    ...
    x = x[:]
    x = x[:,ones(Int64,1,n_py)]
    y = y[:]'
    y = y[ones(Int64,n_px,1),:]
    p = [x[:] y[:]]'
    ...
    ind = reshape(1:n_p,n_px,n_py)
    ind = reshape(ind[1:n_cx,1:n_cy],1,n_c)
    c = [ ind
          ind        + 1
          ind + n_px + 1
          ind + n_px     ]
    ...
    a = zeros(Int64,4,n_c)
    a[1,:] = hcat( zeros(Int64,n_cx)', collect(1:n_cx*(n_cy-1))' )
    a[2,:] = linspace(1,n_c,n_c)+1
    a[2,n_cx:n_cx:n_c] = 0
    a[3,:] = hcat( collect(n_cx+1:n_c)', zeros(Int64,n_cx)' )
    a[4,:] = linspace(1,n_c,n_c)-1
    a[4,1:n_cx:n_c-n_cx+1] = 0


    # Boundary information.
    b = zeros(Int64,5,2*n_cx+2*n_cy)
    b[1,:] = [  collect(1:n_cx)' collect(n_cx:n_cx:n_cx*n_cy)' collect(n_c:-1:n_c-n_cx+1)'  collect(n_c-n_cx+1:-n_cx:1)' ]
    b[2,:] = [  ones(1,n_cx)     2*ones(1,n_cy)                3*ones(1,n_cx)               4*ones(1,n_cy)               ]
    b[3,:] = [  ones(1,n_cx)     2*ones(1,n_cy)                3*ones(1,n_cx)               4*ones(1,n_cy)               ]
    b[4,:] = [ zeros(1,n_cx)       ones(1,n_cy)                 zeros(1,n_cx)                -ones(1,n_cy)               ]
    b[5,:] = [ -ones(1,n_cx)      zeros(1,n_cy)                  ones(1,n_cx)                zeros(1,n_cy)               ]
...
function ap7( kvert )
    ...
    # kcol = zeros(Int64,namax)
    kcol = Array{Int64}(namax)
    kld  = zeros(Int64,neq+1)

    na    = neq
    # kcol1 = zeros(Int64,namax)
    kcol1 = Array{Int64}(namax)
    kcol1[1:neq] = collect(1:neq)
    kind  = zeros(Int64,namax)
...
function assemblef( dcorvg, kvert, kcola, klda, na, neq )
    ...
    for iel = 1:nel
        ...
        for irowl = 1:idfl
            ...
            kentry[irowl,irowl] = ia
            dentry[irowl,irowl] = 0.0
            for jcoll = 1:idfl                # for unsymmetric version
                if jcoll != irowl             # diagonal already processed
        ...
        for icubp = 1:ncubp
            ...
            # summing up over all pairs of multiindices
            for jdofe = 1:idfl
                f[kdfg[jdofe]] = f[kdfg[jdofe]] + aux*dbas[kdfl[jdofe],1]*om
            end
...
function ndfgl( iel, ipar, kvert, kdfg, kdfl, jvg, jvl )
...
function e011( xi1, xi2, djac, bder, dbas, dhelp, detj )
