...
figure, hold on
title('Solver')
plot( res_o(:,1),  res_o(:,8),  '.r-', 'linewidth', lw, 'markersize', mz )
plot( res_m(:,1),  res_m(:,8),  '.g-', 'linewidth', lw, 'markersize', mz )
plot( res_jl(:,1), res_jl(:,8), '.b-', 'linewidth', lw, 'markersize', mz )
plot( res_o(:,1),  res_f(:,8),  '.m-', 'linewidth', lw, 'markersize', mz )
set( gca, 'xtick', res_m(:, 1) )
ylabel('cpu [s]')
xlabel('1/h')
grid on
...
figure, hold on
title('Total time')
plot( res_o(:,1),  sum(res_o(:,2:8),2),  '.r-', 'linewidth', lw, 'markersize', mz )
plot( res_m(:,1),  sum(res_m(:,2:8),2),  '.g-', 'linewidth', lw, 'markersize', mz )
plot( res_jl(:,1), sum(res_jl(:,2:8),2), '.b-', 'linewidth', lw, 'markersize', mz )
plot( res_o(:,1),  sum(res_f(:,2:8),2),  '.m-', 'linewidth', lw, 'markersize', mz )
set( gca, 'xtick', res_m(:, 1) )
ylabel('cpu [s]')
xlabel('1/h')
grid on
...
fprintf(fid,['|---------+',repmat('-----------+',1,7),'-----------|\n']);
fprintf(fid,['|  Octave |',repmat('           |',1,8),'\n']);
fprintf(fid,'|     1/h | %s | %s | %s | %s | %s | %s | %s | %s |\n', '   t_grid', '    t_ptr', '  t_asm_A', '  t_asm_f', '    t_bdr', ' t_sparse', '  t_solve', '    t_tot' );
fprintf(fid,['|---------+',repmat('-----------+',1,7),'-----------|\n']);
for i=1:size(res_jl,1)
  fprintf(fid,'|   %5i | %8.2e | %8.2e | %8.2e | %8.2e | %8.2e | %8.2e | %8.2e | %8.2e |\n', [res_o(i,:), sum(res_o(i,2:end))] );
end
fprintf(fid,['|---------+',repmat('-----------+',1,7),'-----------|\n']);
...
fprintf(fid,['|---------+',repmat('-----------+',1,7),'-----------|\n']);
fprintf(fid,['|  Matlab |',repmat('           |',1,8),'\n']);
fprintf(fid,'|     1/h | %s | %s | %s | %s | %s | %s | %s | %s |\n', '   t_grid', '    t_ptr', '  t_asm_A', '  t_asm_f', '    t_bdr', ' t_sparse', '  t_solve', '    t_tot' );
fprintf(fid,['|---------+',repmat('-----------+',1,7),'-----------|\n']);
for i=1:size(res_jl,1)
  fprintf(fid,'|   %5i | %8.2e | %8.2e | %8.2e | %8.2e | %8.2e | %8.2e | %8.2e | %8.2e |\n', [res_m(i,:), sum(res_m(i,2:end))] );
end
fprintf(fid,['|---------+',repmat('-----------+',1,7),'-----------|\n']);
...
fprintf(fid,['|---------+',repmat('-----------+',1,7),'-----------|\n']);
fprintf(fid,['|   Julia |',repmat('           |',1,8),'\n']);
fprintf(fid,'|     1/h | %s | %s | %s | %s | %s | %s | %s | %s |\n', '   t_grid', '    t_ptr', '  t_asm_A', '  t_asm_f', '    t_bdr', ' t_sparse', '  t_solve', '    t_tot' );
fprintf(fid,['|---------+',repmat('-----------+',1,7),'-----------|\n']);
for i=1:size(res_jl,1)
  fprintf(fid,'|   %5i | %8.2e | %8.2e | %8.2e | %8.2e | %8.2e | %8.2e | %8.2e | %8.2e |\n', [res_jl(i,:), sum(res_jl(i,2:end))] );
end
fprintf(fid,['|---------+',repmat('-----------+',1,7),'-----------|\n']);
...
fprintf(fid,['|---------+',repmat('-----------+',1,7),'-----------|\n']);
fprintf(fid,['| Fortran |',repmat('           |',1,8),'\n']);
fprintf(fid,'|     1/h | %s | %s | %s | %s | %s | %s | %s | %s |\n', '   t_grid', '    t_ptr', '  t_asm_A', '  t_asm_f', '    t_bdr', ' t_sparse', '  t_solve', '    t_tot' );
fprintf(fid,['|---------+',repmat('-----------+',1,7),'-----------|\n']);
for i=1:size(res_f,1)
  fprintf(fid,'|   %5i | %8.2e | %8.2e | %8.2e | %8.2e | %8.2e | %8.2e | %8.2e | %8.2e |\n', [res_f(i,:), sum(res_f(i,2:end))] );
end
fprintf(fid,['|---------+',repmat('-----------+',1,7),'-----------|\n']);
fclose(fid);
...
data = { [res_o(:,1) res_o(:,2) res_m(:,2) res_jl(:,2) res_f(:,2)] ;
         ...
         [res_o(:,1) sum(res_o(:,2:8),2) sum(res_m(:,2:8),2) sum(res_jl(:,2:8),2) sum(res_f(:,2:8),2)] };
title = 'Results and Timings';
filename = [outdir,filesep,'results.html'];
legends = {'Octave','Matlab','Julia','Fortran'};
titles = { 'Grid' 'Matrix Pointers' 'Matrix Assembly' 'RHS Assembly' 'Boundary Conditions' 'Sparse Allocation' 'Solver' 'Total Time' };
